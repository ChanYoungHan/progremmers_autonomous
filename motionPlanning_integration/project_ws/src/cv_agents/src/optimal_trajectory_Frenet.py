...
DF_SET = np.linspace(LANE_WIDTH/2, -LANE_WIDTH/2, LANE_WIDTH, endpoint=True)
...
def calc_frenet_paths(si, si_d, si_dd, sf_d, sf_dd, di, di_d, di_dd, df_d, df_dd, opt_d):
    ...
    for df in DF_SET:
        ...
        d_list_avail.append(frenet_paths[-1].d[-1])
    return frenet_paths, d_list_avail
...
OFFSET_d = 0
def frenet_optimal_planning(pref_line, si, si_d, si_dd, sf_d, sf_dd, di, di_d, di_dd, df_d, df_dd, obs, mapx, mapy, maps, opt_d):
    fplist, d_list_avail = calc_frenet_paths(si, si_d, si_dd, sf_d, sf_dd, di, di_d, di_dd, df_d, df_dd, opt_d)
    fplist = calc_global_paths(fplist, mapx, mapy, maps)

    fplist = check_path(fplist, obs, mapx, mapy, maps)
    # find minimum cost path
    min_cost = float("inf")
    min_cost_pref = float("inf")
    opt_traj = None
    opt_ind = 0
    ## prefer to the line num
    pref_d = DF_SET[pref_line-1]
    pref_ind = None
    ...
    for fp in fplist:
        if min_cost >= fp.c_tot:
            ...
            _opt_ind = opt_ind
        ## prefer to the line
        if fp.d[-1] <= pref_d + OFFSET_d + 0.2  and fp.d[-1] >= pref_d + OFFSET_d - 0.2:
            if min_cost_pref >= fp.c_tot:
                min_cost_pref = fp.c_tot
                pref_traj = fp
                pref_ind = opt_ind
        opt_ind += 1
    if pref_ind == None:
    ...
    try:
        ...
        print(" No solution ! ")
    #print fplist[_opt_ind].d[-1]
    return fplist, _opt_ind, pref_ind, d_list_avail
