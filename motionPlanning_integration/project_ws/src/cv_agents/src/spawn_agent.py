...
def check_obs_distance(x, y, obs):
    edge_point = []
    min_dis = 999999
    edge_point.append( [obs[0][0][0], obs[1][0][0]] )
    edge_point.append( [obs[0][0][0], obs[1][0][-1]] )
    edge_point.append( [obs[0][-1][0], obs[1][0][0]] )
    edge_point.append( [obs[0][-1][0], obs[1][0][-1]] )
    for _x, _y in edge_point:
        dist = (x - _x)**2 + (y - _y)**2
        if dist <= min_dis:
            min_dis = dist
    return min_dis
...
if __name__ == "__main__":
    ...
    d_array_avail = np.array([[0,0,0,0,0]])
    while not rospy.is_shutdown():
        pref_line = 3
        # optimal planning 수행 (output : valid path & optimal path index)
        path, opt_ind, pref_ind, d_list_avail = frenet_optimal_planning(pref_line, si, si_d, si_dd,
                                                sf_d, sf_dd, di, di_d, di_dd, df_d, df_dd, obs, mapx, mapy, traj_s, opt_d)
        #d_state_list.append(di)
        #d_array_avail = np.append(d_array_avail, np.array([d_list_avail]), axis=0)

        ## priority for optimal and preference
        ## implement to check distance with other vehicle
        dist2 = check_obs_distance(state.x, state.y, obs2_xy)
        dist3 = check_obs_distance(state.x, state.y, obs3_xy)
        
        # print(len(path), dist2, dist3)
        # print( len(path), opt_ind, pref_ind)
        if len(path) >= 15 and dist2 > 100 and dist3 > 100:
            opt_ind = pref_ind
        ...
        '''
        si, di = get_frenet(state.x, state.y, mapx, mapy)
        ...
        traj_msg = get_traj_msg(path, opt_ind, mapx, mapy, traj_s)
